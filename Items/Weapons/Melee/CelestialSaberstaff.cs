...
{
    public class CelestialSaberstaff : ModItem
    {
        ...
        public override void SetDefaults()
        {
            ...
            Item.value = Item.sellPrice(0, 40, 45, 0);
        ...
        {
            // Check for existing active GrapheneSaberstaffProjectile2
            bool projectileExists = false;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                // Check if the projectile is active, belongs to the player, and is of the correct type
                if (Main.projectile[i].active && Main.projectile[i].owner == player.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<GrapheneSaberstaffProjectile2>())
                {
            ...
            if (!projectileExists)
            {
                ...
                if (player.altFunctionUse == 2) // If the use is a right-click
                {
                    ...
                    Item.shoot = ModContent.ProjectileType<GrapheneSaberstaffProjectile2>(); // Different projectile for boomerang functionality
                ...
                {
                    Item.shoot = ModContent.ProjectileType<GrapheneSaberstaffProjectile>(); // Different projectile for boomerang functionality
        ...
        {
            CreateRecipe()
            .AddIngredient(null, "Graphene", 15)
