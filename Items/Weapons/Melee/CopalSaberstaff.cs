...
{
    public class CopalSaberstaff : ModItem
    {
        public override void SetDefaults()
        {
            ...
            Item.value = Item.sellPrice(0, 15, 89, 0);
        ...
        {
            // Check for existing active CopalSaberstaffProjectile2
            bool projectileExists = false;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                // Check if the projectile is active, belongs to the player, and is of the correct type
                if (Main.projectile[i].active && Main.projectile[i].owner == player.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<CopalSaberstaffProjectile2>())
                {
            ...
            if (!projectileExists)
            {
                ...
                if (player.altFunctionUse == 2) // If the use is a right-click
                {
                    ...
                    Item.shoot = ModContent.ProjectileType<CopalSaberstaffProjectile2>(); // Different projectile for boomerang functionality
                    Item.shootSpeed = 14f; // Adjust the shoot speed for the boomerang
                    Item.autoReuse = false;
                    return true;
                }
                else // Regular left-click
                {
                    Item.shoot = ModContent.ProjectileType<CopalSaberstaffProjectile>(); // Different projectile for boomerang functionality
