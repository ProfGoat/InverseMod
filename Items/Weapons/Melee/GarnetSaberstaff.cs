...
{
    public class GarnetSaberstaff : ModItem
    {
        public override void SetDefaults()
        {
            ...
            Item.value = Item.sellPrice(0, 9, 76, 0);
        ...
        {
            // Check for existing active GarnetSaberstaffProjectile2
            bool projectileExists = false;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                // Check if the projectile is active, belongs to the player, and is of the correct type
                if (Main.projectile[i].active && Main.projectile[i].owner == player.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<GarnetSaberstaffProjectile2>())
                {
            ...
            if (!projectileExists)
            {
                ...
                if (player.altFunctionUse == 2) // If the use is a right-click
                {
                    ...
                    Item.shoot = ModContent.ProjectileType<GarnetSaberstaffProjectile2>(); // Different projectile for boomerang functionality
                    Item.shootSpeed = 12f; // Adjust the shoot speed for the boomerang
                    Item.autoReuse = false;
                    return true;
                }
                else // Regular left-click
                {
                    Item.shoot = ModContent.ProjectileType<GarnetSaberstaffProjectile>(); // Different projectile for boomerang functionality
