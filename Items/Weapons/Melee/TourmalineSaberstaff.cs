...
{
    public class TourmalineSaberstaff : ModItem
    {
        ...
        {
            // Check for existing active TourmalineSaberstaffProjectile2
            bool projectileExists = false;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                // Check if the projectile is active, belongs to the player, and is of the correct type
                if (Main.projectile[i].active && Main.projectile[i].owner == player.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<TourmalineSaberstaffProjectile2>())
                {
            ...
            if (!projectileExists)
            {
                ...
                if (player.altFunctionUse == 2) // If the use is a right-click
                {
                    ...
                    Item.shoot = ModContent.ProjectileType<TourmalineSaberstaffProjectile2>(); // Different projectile for boomerang functionality
                    Item.shootSpeed = 9f; // Adjust the shoot speed for the boomerang
                    Item.autoReuse = false;
                    return true;
                }
                else // Regular left-click
                {
                    Item.shoot = ModContent.ProjectileType<TourmalineSaberstaffProjectile>(); // Different projectile for boomerang functionality
        ...
        {
            CreateRecipe()
            .AddIngredient(null, "Tourmaline", 15)
