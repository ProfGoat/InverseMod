...
{
    public class JadeSaberstaff : ModItem
    {
        ...
        {
            // Check for existing active JadeSaberstaffProjectile2
            bool projectileExists = false;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                // Check if the projectile is active, belongs to the player, and is of the correct type
                if (Main.projectile[i].active && Main.projectile[i].owner == player.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<JadeSaberstaffProjectile2>())
                {
            ...
            if (!projectileExists)
            {
                ...
                if (player.altFunctionUse == 2) // If the use is a right-click
                {
                    ...
                    Item.shoot = ModContent.ProjectileType<JadeSaberstaffProjectile2>(); // Different projectile for boomerang functionality
                    Item.shootSpeed = 11f; // Adjust the shoot speed for the boomerang
                    Item.autoReuse = false;
                    return true;
                }
                else // Regular left-click
                {
                    Item.shoot = ModContent.ProjectileType<JadeSaberstaffProjectile>(); // Different projectile for boomerang functionality
